public class Node {
    ...
    public Node right;
    public String type;
    public String nodeType; // puede ser Operation: 1, Leaf: 2, Declaration: 3, Funcion: 4, VarOP(Variable en expresion): 5, VarDcl (Variable declarada): 6
        ...
        try {
            switch (type) {
                case "int":
                    ...
                    if (op.matches("-?\\d+")) valor = Integer.parseInt(op);
                    break;
                case "float":
                    // acepta formatos con punto y/o exponencial
                    if (op.matches("-?\\d+(\\.\\d+)?([eE]-?\\d+)?")) valor = Float.parseFloat(op);
                    break;
                case "boolean":
                    if (op.equals("true") || op.equals("false")) valor = Boolean.parseBoolean(op);
                    break;
                default:
            ...
            // si no se puede parsear, dejar valor null (por ejemplo, identificador usado por error aquí)
            valor = null;
        }
    }
        ...
        switch (nodeType) {
            ...
            case "VarOP":
                if(this.type == null) {
                    ...
                    if(listElement!=null){                    
                        System.out.println("variable encontrada en tabla : " + listElement.getId());
                        return listElement.getType();
                    }else{
